...
import {
  ...
  ChevronDown, ChevronRight, ExternalLink, Send, CheckCircle2, Clock,
  Circle, Instagram, Youtube, Mail, Phone, MapPin, Filter,
  TrendingUp, Zap, Heart, BookOpen, Music, Dumbbell, Code2,
  Award, Loader2, X, ChevronUp, Play, Eye, AlignLeft, User
} from "lucide-react";
...
/* ═══════════════════════════════════════════════════════════════════════ */
export default function OsisPage({ theme }: OsisPageProps) {
  ...
  return (
    <div className={`min-h-screen ${bg} ${textPrimary}`}>
      ...
      {/* ── Sticky Section Nav ─────────────────────────────────────────── */}
      <nav className={`sticky top-0 z-40 backdrop-blur-xl border-b ${isDark ? "bg-slate-950/90 border-slate-800" : "bg-white/90 border-slate-200"} shadow-sm`}>
        <div className="max-w-7xl mx-auto px-4">
          {/* Desktop nav */}
          <div className="hidden md:flex items-center gap-1 overflow-x-auto py-2 scrollbar-hide">
            <button
              onClick={() => navigate("/")}
              className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium shrink-0 ${textMuted} hover:text-blue-500 transition-colors`}
            >
          ...
          <div className="md:hidden flex items-center justify-between py-2">
            ...
            <span className={`text-xs font-bold text-blue-500`}>OSIS SKANSAGIRI</span>
                  ...
                  {ANCHORS.map(a => (
                    <button key={a.id} onClick={() => scrollTo(a.id)}
                      className={`px-2 py-1.5 rounded-lg text-xs font-semibold transition-all text-center ${
                        activeSection === a.id ? "bg-blue-600 text-white" : `${textMuted} hover:text-blue-500`
                      }`}
                    >{a.label}</button>
                  ))}
                </div>
      ...
      <section id="hero" className="relative min-h-screen flex items-center overflow-hidden">
        ...
        <div className="absolute inset-0">
          ...
          <div className="absolute bottom-1/4 right-1/4 w-96 h-96 rounded-full bg-indigo-600/15 blur-3xl" />
        </div>
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20 w-full">
          <div className="text-center">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-blue-500/10 border border-blue-500/30 text-blue-400 text-xs font-bold tracking-widest uppercase mb-8">
            ...
            <motion.h1 initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }}
              className="text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-black text-white mb-4 leading-none tracking-tight">
              {info?.namaKabinet || "OSIS SKANSAGIRI"}
            </motion.h1>
            <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
              className="text-lg sm:text-xl text-blue-300/80 font-light mb-2">
              SMKN 1 Wonogiri
            </motion.p>
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4 }}
              ...
              <button onClick={() => scrollTo("aspirasi")}
                className="px-6 py-3 rounded-xl bg-blue-600 hover:bg-blue-500 text-white font-bold text-sm tracking-wide transition-all hover:scale-105 active:scale-95 flex items-center justify-center gap-2">
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.5 }}
              ...
              {[
                { val: info?.jumlahProker || proker.length, label: "Program Kerja" },
                { val: info?.jumlahMember || pengurus.length, label: "Anggota Pengurus" },
                { val: info?.jumlahEkskul || ekskul.length, label: "Ekstrakurikuler" },
              ].map((s, i) => (
                <div key={i} className="flex flex-col items-center p-4 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-sm">
                  <span className="text-3xl sm:text-4xl font-black text-blue-400">{s.val}</span>
                  <span className="text-xs text-slate-400 mt-1 text-center">{s.label}</span>
                </div>
              ))}
            </motion.div>
          </div>
        ...
        {nearestEvent && !countdown.passed && (
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.7 }}
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-6 px-6 py-3 rounded-2xl bg-white/5 border border-white/10 backdrop-blur-md text-white">
              <span className="text-xs text-slate-400 tracking-widest uppercase">Event terdekat: <span className="text-blue-400 font-semibold">{nearestEvent.nama}</span></span>
              <div className="flex items-center gap-2">
                {[["Hari", countdown.days], ["Jam", countdown.hours], ["Menit", countdown.minutes], ["Detik", countdown.seconds]].map(([l, v], i) => (
                  <div key={i} className="flex flex-col items-center">
                    ...
                    <span className="text-[9px] text-slate-500 uppercase tracking-wider">{l}</span>
                  </div>
                ))}
              </div>
            </div>
      ...
      {/* 2. TENTANG OSIS                                                  */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section id="tentang" className="py-20 px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-6xl mx-auto">
          <SectionHeader title="Tentang OSIS" subtitle="Mengenal lebih dekat organisasi kami" />
          <div className="grid md:grid-cols-2 gap-8">
            {/* Sejarah & Quote */}
            <div className="space-y-6">
              <div className={`p-6 rounded-2xl border ${cardBg}`}>
                <h3 className={`text-lg font-bold mb-3 text-blue-500`}>Sejarah Singkat</h3>
              ...
              {info?.quoteKetua && (
                <div className="p-6 rounded-2xl bg-gradient-to-br from-blue-600/10 to-indigo-600/10 border border-blue-500/20">
                  <blockquote className={`italic text-sm leading-relaxed ${textMuted} mb-3`}>"{info.quoteKetua}"</blockquote>
                  <p className="text-blue-400 font-semibold text-sm">— {info.namaKetua || "Ketua OSIS"}</p>
                </div>
              )}
              <div className={`p-4 rounded-xl border ${cardBg}`}>
                <p className={`text-xs ${textMuted} mb-2 font-semibold uppercase tracking-wider`}>Landasan Hukum</p>
                <p className={`text-xs ${textMuted}`}>Permendikbud No. 23 Tahun 2015 tentang Penumbuhan Budi Pekerti dan Permendiknas No. 39 Tahun 2008 tentang Pembinaan Kesiswaan.</p>
              </div>
            </div>

            {/* Visi Misi */}
            <div className={`p-6 rounded-2xl border ${cardBg}`}>
              <h3 className="text-lg font-bold mb-4 text-blue-500">Visi & Misi</h3>
              {info?.visi && (
                <div className="mb-5">
                  <p className={`text-xs ${textMuted} uppercase tracking-widest mb-2 font-bold`}>Visi</p>
                  <p className={`${textMuted} text-sm leading-relaxed`}>{info.visi}</p>
                </div>
              )}
              {info?.misi && info.misi.length > 0 && (
                <div>
                  <p className={`text-xs ${textMuted} uppercase tracking-widest mb-3 font-bold`}>Misi</p>
      ...
      {/* ══════════════════════════════════════════════════════════════════ */}
      {/* 3. PENGURUS — Interactive Org Chart                              */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section id="pengurus" className={`py-20 px-4 sm:px-6 lg:px-8 relative z-10 ${isDark ? "bg-slate-900/30" : "bg-blue-50/30"}`}>
        <div className="max-w-6xl mx-auto">
          <SectionHeader title="Struktur Pengurus" subtitle="Tim yang memimpin OSIS periode ini" />
          {pengurus.length === 0 ? (
            <EmptyState icon={Users} text="Data pengurus belum tersedia." />
          ) : (
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
              {pengurus.sort((a,b) => a.urutan - b.urutan).map((p, i) => (
                <motion.button key={p.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                  transition={{ delay: i * 0.05 }} viewport={{ once: true }}
                  onClick={() => setSelectedPengurus(p)}
                  className={`flex flex-col items-center p-4 rounded-2xl border transition-all hover:scale-105 hover:border-blue-500/50 hover:shadow-blue-500/10 hover:shadow-lg cursor-pointer text-center ${cardBg}`}>
                  <div className="w-16 h-16 rounded-2xl overflow-hidden mb-3 bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center flex-shrink-0">
                    {p.foto ? <img src={p.foto} alt={p.nama} className="w-full h-full object-cover" /> : <User className="w-7 h-7 text-white" />}
                  </div>
                  <p className={`text-xs font-bold ${textPrimary} leading-tight mb-1`}>{p.nama}</p>
                  <p className="text-[10px] text-blue-400 font-medium leading-tight">{p.jabatan}</p>
                  {p.bidang && <p className={`text-[9px] ${textMuted} mt-1`}>{p.bidang}</p>}
                </motion.button>
              ))}
            </div>
          )}
          ...
          {selectedPengurus && (
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
              ...
              <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
                className={`w-full max-w-md p-6 rounded-3xl border shadow-2xl ${cardBg}`}
                onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-start mb-4">
                  <div className="flex items-center gap-3">
                    <div className="w-14 h-14 rounded-2xl overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center">
                    ...
                    <div>
                      <p className={`font-bold ${textPrimary}`}>{selectedPengurus.nama}</p>
                      <p className="text-blue-400 text-sm">{selectedPengurus.jabatan}</p>
                      {selectedPengurus.bidang && <p className={`text-xs ${textMuted}`}>{selectedPengurus.bidang}</p>}
                    </div>
                  </div>
                  <button onClick={() => setSelectedPengurus(null)} className={`p-2 rounded-lg hover:bg-slate-700/50 ${textMuted}`}><X className="w-4 h-4" /></button>
                </div>
                {selectedPengurus.tugasPokok && (
                  <div className="mb-4">
                    <p className={`text-xs ${textMuted} uppercase tracking-widest mb-2 font-bold`}>Tugas Pokok</p>
                ...
                <div className="flex gap-2">
                  {selectedPengurus.instagram && (
                    <a href={`https://instagram.com/${selectedPengurus.instagram.replace("@","")}`} target="_blank" rel="noopener noreferrer"
                      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-pink-500/10 text-pink-400 text-xs font-medium hover:bg-pink-500/20 transition-colors">
                      <Instagram className="w-3.5 h-3.5" /> Instagram
                    </a>
                  )}
                  {selectedPengurus.email && (
                    <a href={`mailto:${selectedPengurus.email}`}
                      className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-blue-500/10 text-blue-400 text-xs font-medium hover:bg-blue-500/20 transition-colors">
      ...
      {/* ══════════════════════════════════════════════════════════════════ */}
      {/* 4. PROGRAM KERJA — Kanban                                        */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section id="proker" className="py-20 px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-6xl mx-auto">
          <SectionHeader title="Program Kerja" subtitle="Realisasi visi melalui aksi nyata" />
            ...
            {["SEMUA", ...BIDANG_PROKER].map(b => (
              <button key={b} onClick={() => setProkerBidang(b)}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                  prokerBidang === b ? "bg-blue-600 text-white" : `border ${isDark ? "border-slate-700 text-slate-400 hover:border-blue-500" : "border-slate-300 text-slate-500 hover:border-blue-400"}`
                }`}>{b === "SEMUA" ? "Semua Bidang" : b}</button>
            ))}
          </div>

          {proker.length === 0 ? (
            <EmptyState icon={Target} text="Program kerja belum tersedia." />
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {prokerFiltered.sort((a,b) => a.urutan - b.urutan).map((pk, i) => {
                const cfg = STATUS_CFG[pk.status] || STATUS_CFG.DIRENCANAKAN;
                return (
                  <motion.div key={pk.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                    transition={{ delay: i * 0.04 }} viewport={{ once: true }}
                    className={`p-5 rounded-2xl border ${cardBg}`}>
                    <div className="flex items-start justify-between mb-3">
                      <div className="flex-1">
                        <span className={`text-[9px] font-bold uppercase tracking-widest px-2 py-0.5 rounded-full border ${isDark ? "border-blue-500/30 text-blue-400" : "border-blue-400/50 text-blue-600"} bg-blue-500/10`}>{pk.bidang}</span>
                        <h4 className={`mt-2 text-sm font-bold ${textPrimary} leading-tight`}>{pk.nama}</h4>
                      </div>
                      <span className={`ml-2 shrink-0 text-[10px] font-semibold px-2 py-1 rounded-full ${cfg.cls}`}>{cfg.label}</span>
                    </div>
                    {pk.deskripsi && <p className={`text-xs ${textMuted} leading-relaxed mb-3`}>{pk.deskripsi}</p>}
                    {/* Progress bar */}
                    <div className="mb-2">
                      ...
                      <div className={`w-full h-1.5 rounded-full ${isDark ? "bg-slate-800" : "bg-slate-200"}`}>
                        <div className="h-full rounded-full bg-gradient-to-r from-blue-500 to-indigo-500 transition-all duration-700"
                          style={{ width: `${pk.progress}%` }} />
                      </div>
                    </div>
                    <div className="flex items-center justify-between text-[10px]">
                      {pk.targetDate && <span className={textMuted}>Target: {new Date(pk.targetDate).toLocaleDateString("id-ID", { day:"numeric", month:"short", year:"numeric"})}</span>}
                      {pk.penanggungJawab && <span className={`${textMuted}`}>PJ: {pk.penanggungJawab}</span>}
                    </div>
                  </motion.div>
                );
              })}
      ...
      {/* ══════════════════════════════════════════════════════════════════ */}
      {/* 5. AGENDA & EVENT CALENDAR                                       */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section id="agenda" className={`py-20 px-4 sm:px-6 lg:px-8 relative z-10 ${isDark ? "bg-slate-900/30" : "bg-blue-50/30"}`}>
        <div className="max-w-6xl mx-auto">
          <SectionHeader title="Agenda & Event" subtitle="Jadwal kegiatan OSIS yang akan datang" />
          {agenda.length === 0 ? (
            <EmptyState icon={Calendar} text="Belum ada agenda yang dijadwalkan." />
          ) : (
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {agenda.sort((a,b) => new Date(a.tanggal).getTime() - new Date(b.tanggal).getTime()).map((ev, i) => {
                ...
                return (
                  <motion.div key={ev.id} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }}
                    transition={{ delay: i * 0.05 }} viewport={{ once: true }}
                    className={`p-5 rounded-2xl border transition-all ${cardBg} ${isPast ? "opacity-60" : "hover:border-blue-500/40"}`}>
                    <div className="flex items-start gap-4">
                      <div className="shrink-0 flex flex-col items-center justify-center w-12 h-14 rounded-xl bg-blue-600/20 text-blue-400">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2 mb-1 flex-wrap">
                          ...
                          {isPast && <span className="text-[9px] text-slate-500 border border-slate-600/40 px-1.5 py-0.5 rounded-full">Lewat</span>}
                        </div>
                        ...
                        {ev.deskripsi && <p className={`text-xs ${textMuted} mt-1 leading-relaxed line-clamp-2`}>{ev.deskripsi}</p>}
                      </div>
                    </div>
                  </motion.div>
                );
      ...
      {/* 6. EKSTRAKURIKULER                                               */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section id="ekskul" className="py-20 px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-6xl mx-auto">
          <SectionHeader title="Ekstrakurikuler" subtitle="Temukan minat dan bakatmu" />
          {/* Filter */}
          <div className="flex gap-2 flex-wrap mb-8 justify-center">
              ...
              return (
                <button key={k} onClick={() => setEkskulFilter(k)}
                  className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                    ekskulFilter === k ? "bg-blue-600 text-white" : `border ${isDark ? "border-slate-700 text-slate-400 hover:border-blue-500" : "border-slate-300 text-slate-500 hover:border-blue-400"}`
                  }`}>
          ...
          {ekskul.length === 0 ? (
            <EmptyState icon={Zap} text="Data ekstrakurikuler belum tersedia." />
          ) : (
              ...
              {ekskulFiltered.sort((a,b) => a.urutan - b.urutan).map((ex, i) => {
                ...
                return (
                  <motion.div key={ex.id} initial={{ opacity: 0, scale: 0.95 }} whileInView={{ opacity: 1, scale: 1 }}
                    transition={{ delay: i * 0.05 }} viewport={{ once: true }}
                    className={`overflow-hidden rounded-2xl border ${cardBg} hover:border-blue-500/40 transition-all hover:scale-[1.02]`}>
                    {ex.foto ? (
                      <img src={ex.foto} alt={ex.nama} className="w-full h-36 object-cover" />
                    ) : (
                      <div className="w-full h-24 bg-gradient-to-br from-blue-600/20 to-indigo-600/20 flex items-center justify-center">
                        <Icon className="w-10 h-10 text-blue-400/50" />
                      </div>
                    )}
                    <div className="p-4">
                      ...
                      {ex.deskripsi && <p className={`text-xs ${textMuted} leading-relaxed mb-3 line-clamp-2`}>{ex.deskripsi}</p>}
      ...
      {/* ══════════════════════════════════════════════════════════════════ */}
      {/* 7. GALERI KEGIATAN OSIS                                         */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section id="galeri" className={`py-20 px-4 sm:px-6 lg:px-8 relative z-10 ${isDark ? "bg-slate-900/30" : "bg-blue-50/30"}`}>
        <div className="max-w-6xl mx-auto">
          <SectionHeader title="Galeri Kegiatan" subtitle="Dokumentasi momen bersejarah OSIS" />
          <div className="flex gap-2 flex-wrap mb-6 justify-center">
            {galeriCategories.map(k => (
              <button key={k} onClick={() => setGaleriFilter(k)}
                className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-all ${
                  galeriFilter === k ? "bg-blue-600 text-white" : `border ${isDark ? "border-slate-700 text-slate-400 hover:border-blue-500" : "border-slate-300 text-slate-500 hover:border-blue-400"}`
                }`}>{k === "SEMUA" ? "Semua" : k}</button>
            ))}
          </div>

          {galeri.length === 0 ? (
            <EmptyState icon={Camera} text="Galeri kegiatan belum tersedia." />
          ) : (
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
            ...
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }}
              ...
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-black/80 rounded-b-2xl">
                ...
                <p className="text-slate-400 text-sm">{lightbox.kategori}</p>
              </div>
              <button onClick={() => setLightbox(null)} className="absolute top-2 right-2 p-2 rounded-full bg-black/50 text-white hover:bg-black/80 transition-colors"><X className="w-5 h-5" /></button>
            </motion.div>
          </motion.div>
        )}
      </AnimatePresence>
      ...
      {/* 8. PRESTASI                                                      */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section id="prestasi" className="py-20 px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-4xl mx-auto">
          <SectionHeader title="Prestasi" subtitle="Capaian membanggakan OSIS & Ekstrakurikuler" />
          {prestasi.length === 0 ? (
            <EmptyState icon={Trophy} text="Belum ada data prestasi." />
          ) : (
            <div className="relative">
              <div className="absolute left-4 md:left-1/2 top-0 bottom-0 w-px bg-blue-500/20" />
              <div className="space-y-6">
                {prestasi.sort((a,b) => new Date(b.tanggal).getTime() - new Date(a.tanggal).getTime()).map((p, i) => {
                  ...
                  const isLeft = i % 2 === 0;
                  return (
                    <motion.div key={p.id} initial={{ opacity: 0, x: isLeft ? -20 : 20 }}
                      whileInView={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }} viewport={{ once: true }}
                      className={`relative flex items-center gap-4 md:gap-8 ${isLeft ? "md:flex-row" : "md:flex-row-reverse"}`}>
                      <div className="absolute left-4 md:left-1/2 w-3 h-3 rounded-full bg-blue-500 border-2 border-blue-300 -translate-x-1/2 md:-translate-x-1/2 shadow-lg shadow-blue-500/40 z-10" />
                      <div className={`pl-10 md:pl-0 md:w-1/2 ${isLeft ? "md:pr-8 md:text-right" : "md:pl-8"}`}>
                        <div className={`inline-flex p-4 rounded-2xl border ${cardBg} ${isLeft ? "md:ml-auto" : ""} max-w-sm`}>
                          {p.foto && <img src={p.foto} alt={p.judul} className="w-16 h-16 object-cover rounded-xl mr-3 flex-shrink-0" />}
                          <div>
                            <div className="flex items-center gap-2 mb-1 flex-wrap">
      ...
      {/* 9. ASPIRASI SISWA                                                */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section id="aspirasi" className={`py-20 px-4 sm:px-6 lg:px-8 relative z-10 ${isDark ? "bg-slate-900/30" : "bg-blue-50/30"}`}>
        <div className="max-w-6xl mx-auto">
          <SectionHeader title="Aspirasi Siswa" subtitle="Suaramu penting — OSIS mendengar" />
          <div className="grid lg:grid-cols-2 gap-8">
            {/* Form */}
            <div className={`p-6 rounded-2xl border ${cardBg}`}>
              <h3 className="font-bold text-blue-400 mb-4 flex items-center gap-2"><MessageSquare className="w-4 h-4" /> Kirim Aspirasi</h3>
              {aspSuccess ? (
                ...
                <form onSubmit={handleAspirasi} className="space-y-4">
                  ...
                  {!aspAnonim && (
                    <div className="grid sm:grid-cols-2 gap-3">
                      <input value={aspNama} onChange={e => setAspNama(e.target.value)}
                        placeholder="Nama lengkap" className={inputCls(isDark)} />
                      <input value={aspKelas} onChange={e => setAspKelas(e.target.value)}
                        placeholder="Kelas (mis. XII TKJ 1)" className={inputCls(isDark)} />
                    </div>
                  )}
            ...
            {/* Public aspirasi jawaban */}
            <div className="space-y-3">
              <h3 className={`font-bold ${textPrimary} mb-3 flex items-center gap-2`}><Eye className="w-4 h-4 text-blue-400" /> Aspirasi yang Telah Dijawab</h3>
              {aspirasiPublik.length === 0 ? (
                <div className={`p-6 rounded-2xl border ${cardBg} text-center`}>
                  <p className={`text-sm ${textMuted}`}>Belum ada aspirasi publik yang tersedia.</p>
                </div>
              ) : (
                aspirasiPublik.map((asp, i) => (
                  <motion.div key={asp.id} initial={{ opacity: 0, y: 10 }} whileInView={{ opacity: 1, y: 0 }}
                    transition={{ delay: i * 0.05 }} viewport={{ once: true }}
                    ...
                    <p className={`text-sm ${textMuted} mb-2`}>{asp.isi}</p>
                    {asp.balasan && (
      ...
      {/* ══════════════════════════════════════════════════════════════════ */}
      {/* 10. KONTAK & FOLLOW OSIS                                        */}
      {/* ══════════════════════════════════════════════════════════════════ */}
      <section id="kontak" className="py-20 px-4 sm:px-6 lg:px-8 relative z-10">
        <div className="max-w-4xl mx-auto text-center">
          <SectionHeader title="Kontak & Follow OSIS" subtitle="Tetap terhubung bersama kami" />
          <div className="grid sm:grid-cols-3 gap-4 mb-8">
            {[
              ...
              <a key={label} href={href} target="_blank" rel="noopener noreferrer"
                className={`flex flex-col items-center p-5 rounded-2xl bg-gradient-to-br ${color} text-white hover:scale-105 transition-transform`}>
          ...
          <div className={`p-6 rounded-2xl border ${cardBg} text-left`}>
            <h3 className={`font-bold ${textPrimary} mb-4 flex items-center gap-2`}><Calendar className="w-4 h-4 text-blue-400" /> Jadwal Audiensi Siswa</h3>
          ...
          <div className="mt-6">
            <button onClick={() => navigate("/osis/adm-panel")}
              className={`text-xs ${textMuted} hover:text-blue-400 transition-colors`}>
              Admin Panel →
            </button>
          </div>
        </div>
